...
function TeamPicker({ sport, value, onChange }) {
    ...
    <div style={{ position: 'relative' }}>
      <button type="button" onClick={() => setOpen(o => !o)} style={{
        ...
        {selected
          ? <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 18, height: 18, borderRadius: 4, background: selected.secondary, display: 'inline-block', border: '1px solid rgba(255,255,255,0.2)' }} />
              {selected.name}
            </span>
      ...
      {open && (
        <div style={{
          ...
          {teams.map(team => (
            <button key={team.abbr} type="button" onClick={() => { onChange(team.abbr); setOpen(false); }} style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: 10,
              padding: '9px 12px', background: value === team.abbr ? team.primary + '33' : 'transparent',
...
export default function Lobby({ onCreateRoom, onJoinRoom, llmStatus }) {
  ...
  useEffect(() => {
    ...
    if (prefs.teamAbbr) setTeamAbbr(prefs.teamAbbr);
    if (prefs.location) setLocation(prefs.location);
    if (prefs.botCount !== undefined) setBotCount(prefs.botCount);
  }, []);

  const canCreate = name.trim().length >= 2;
  const canJoin   = name.trim().length >= 2 && joinCode.trim().length === 4;

  const handleSportChange = (s) => { setSport(s); setTeamAbbr(''); };

  const persistPrefs = (overrides = {}) => {
    savePreferences({ name: name.trim(), sport, teamAbbr, location, botCount, ...overrides });
    setSavedBadge(true);
  ...
  const handleCreate = async () => {
    ...
    persistPrefs();
    try {
      await onCreateRoom({ name: name.trim(), sport, team: teamAbbr, location, botCount });
  ...
  const handleJoin = async () => {
    ...
    setLoading(true);
    savePreferences({ name: name.trim(), teamAbbr, sport });
    try {
      await onJoinRoom({ name: name.trim(), team: teamAbbr, code: joinCode.trim().toUpperCase() });
    } finally { setLoading(false); }
  };

  const selectedTeam = (TEAMS_BY_SPORT[sport] || {})[teamAbbr];
    ...
    <div className="page">
      ...
      <div className="lobby-form">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div className="form-group">
              ...
              <div className="segment-group">
                {SPORT_OPTIONS.map(s => (
                  <button key={s.value} className={`segment-btn ${sport === s.value ? 'active' : ''}`}
            ...
            <div className="form-group">
              ...
              <TeamPicker sport={sport} value={teamAbbr} onChange={setTeamAbbr} />
              {selectedTeam && (
                <div style={{
                  marginTop: 4, padding: '5px 10px', borderRadius: 8,
                  background: `${selectedTeam.primary}18`,
                  border: `1px solid ${selectedTeam.primary}33`,
                  fontSize: 11, color: 'var(--text-muted)',
                }}>
                  🎨 Card in {selectedTeam.name} colors
                  {sport === 'hockey' && ' · NHL feed auto-marks your game'}
                </div>
              )}
            </div>

            <button className="btn btn-secondary btn-sm" onClick={() => persistPrefs()}
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {LOCATION_OPTIONS.map(l => (
                  <button key={l.value} onClick={() => setLocation(l.value)} style={{
                    ...
                    {l.camera && (
                      <span style={{ fontSize: 11, color: 'var(--gold)' }}>📷 Camera squares</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="form-group">
              ...
              <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
                Add bots if you don't have enough players. Bots auto-mark squares and fire battle shots!
              </p>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {llmStatus === 'fallback' && (
                  <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>Using static squares</span>
                )}
              </div>
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>
                Each game gets fresh, unique squares tailored to the matchup, teams, and your location.
                {locOption?.camera && ' 📷 Camera squares included for live/bar attendance.'}
              </p>
            </div>
        ...
        {mode === 'join' && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center' }}>
              You'll get your own AI-generated card in your team's colors. Sport and location match the host's.
            </p>
        ...
        <div className="card" style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 2 }}>
          ...
          <p>✨ <strong style={{ color: 'var(--text)' }}>AI generates unique squares</strong> for every new game</p>
          <p>🏒 <strong style={{ color: 'var(--text)' }}>NHL games auto-mark</strong> from the live feed</p>
          <p>📷 <strong style={{ color: 'var(--gold)' }}>Camera squares</strong> require a photo verified by AI</p>
          <p>⚡ <strong style={{ color: 'var(--battle)' }}>Battle squares</strong> → BINGO earns a Battle Shot</p>
          <p>💣 <strong>Battle Shots</strong> block an opponent's square</p>
          <p>🤖 <strong>Bots compete</strong> when you're short on players</p>
        </div>
      </div>
    </div>
  );
